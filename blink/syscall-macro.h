...
#define SYSCALL1(magnum, name, func, signature)                  \
  case magnum:                                                   \
    if (STRACE && ST_HAS(signature, IS_MEM_IO) && FLAG_strace) { \
      EnterStrace(m, signature, di);                             \
    }                                                            \
    ax = func(m, di);                                            \
    if (STRACE && FLAG_strace) {                                 \
...
#define SYSCALL3(magnum, name, func, signature)                  \
  case magnum:                                                   \
    if (STRACE && ST_HAS(signature, IS_MEM_IO) && FLAG_strace) { \
      EnterStrace(m, signature, di, si, dx);                     \
    }                                                            \
    ax = func(m, di, si, dx);                                    \
    if (STRACE && FLAG_strace) {                                 \
...
#define SYSCALL4(magnum, name, func, signature)                  \
  case magnum:                                                   \
    if (STRACE && ST_HAS(signature, IS_MEM_IO) && FLAG_strace) { \
      EnterStrace(m, signature, di, si, dx, r0);                 \
    }                                                            \
...
#define SYSCALL5(magnum, name, func, signature)                  \
  case magnum:                                                   \
    if (STRACE && ST_HAS(signature, IS_MEM_IO) && FLAG_strace) { \
      EnterStrace(m, signature, di, si, dx, r0, r8);             \
    }                                                            \
...
#define SYSCALL6(magnum, name, func, signature)                    \
  case magnum:                                                     \
    if (STRACE && ST_HAS(signature, IS_MEM_IO) && FLAG_strace) {   \
      EnterStrace(m, signature, di, si, dx, r0, r8, r9);           \
    }                                                              \
