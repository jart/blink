...
void OpDas(struct Machine *m, DISPATCH_PARAMETERS) {
  u8 al, af, cf;
  af = cf = 0;
  al = m->al;
...
void OpAaa(struct Machine *m, DISPATCH_PARAMETERS) {
  u8 af, cf;
  af = cf = 0;
...
void OpAas(struct Machine *m, DISPATCH_PARAMETERS) {
  u8 af, cf;
  af = cf = 0;
  if ((m->al & 0x0f) > 9 || GetFlag(m->flags, FLAGS_AF)) {
    cf = m->al < 6 || GetFlag(m->flags, FLAGS_CF);
...
void OpAam(struct Machine *m, DISPATCH_PARAMETERS) {
  u8 i = uimm0;
  if (!i) return RaiseDivideError(m);
  m->ah = m->al / i;
  m->al = m->al % i;
  AluFlags8(m->al, 0, &m->flags, 0, 0);
}

void OpAad(struct Machine *m, DISPATCH_PARAMETERS) {
  u8 i = uimm0;
